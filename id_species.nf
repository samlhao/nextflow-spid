...
========================================================================================
                         nf-core/spid
========================================================================================
 nf-core/spid Species ID Pipeline.
...
def helpMessage() {
    ...
    nextflow run nf-core/spid --pe_reads 'data/*_{1,2}.fastq.gz' --readsdir results/reads --output id_species.tsv -profile docker

    Mandatory arguments:
      --output 	                    Name of output file with species IDs
      --readsdir                    Output directory where reads will be organized by species
...
// Create channel for output file
// output_ch = file (params.output)
...
if (params.se_reads) {
    ...
    if (params.skip_trimming) {
        se_read_files_trimming.into(se_bbsketch_ch)
    }
}
else {
    Channel
...
if (params.pe_reads) {
    ...
    if (params.skip_trimming) {
        pe_read_files_trimming.into(pe_bbsketch_ch)
    }
}
else {
    Channel
...
if (params.assemblies) {
    Channel
        .fromFilePairs(params.assemblies, size: 1, checkIfExists: true)
...
process se_fastp {
    tag "$sample_id"
    label 'process_small'
    publishDir "${params.outdir}/samples/${sample_id}/${sample_id}_reads", mode: 'copy'

    input:
    tuple(sample_id, path(reads)) from se_read_files_trimming

    output:
    tuple (sample_id, path('*_trimmed.fq.gz'), path(reads)) into se_bbsketch_ch

    when:
    params.se_reads && !params.skip_trimming
...
process pe_fastp {
    ...
    output:
    tuple (sample_id, path('*_trimmed.fq.gz'), path(reads)) into pe_bbsketch_ch

    when:
    params.pe_reads && !params.skip_trimming
...
process multiqc {
    ...
    """
    multiqc -f $rtitle $rfilename --config $multiqc_config . fastqc/*
    """
}
